...
// operation ControlledVGate(control: Qubit, target: Qubit) : Unit {
//     body (...){
//         let V = Exp([PauliX], 0.5, _);
//         Controlled V([control], [target]);
//     }
// }


// operation ControlledVTransposeGate(control: Qubit, target: Qubit) : Unit {
//     body(...) {
//         let VTranspose = Adjoint Exp([PauliX], 0.5, _);

//         Controlled VTranspose([control], [target]);
//     }
// }
    ...
    operation multiplyH(x : Int[], y : Int[]) : Int[]{
        ...
        mutable res = [];
        for i in 0..9{
            set res w/= i <- ResultArrayAsInt([M(resQ[i])]);
        }
        return res;
    }
    @EntryPoint()
    operation main() : Int[] {
        ...
        let carry = ResultArrayAsInt([M(res)]);
        let sum = ResultArrayAsInt([M(qC)]);

        ResetAll([qA, qB, qC, res]);
    ...
    //     Controlled Rx([a], (PI()/2.0, b));
    //     Controlled Rx([a], (PI()/2.0, b));
    //     Controlled Rx([a], (-PI()/2.0, b));

    //     let result = ResultArrayAsInt([M(b)]);
    //     ResetAll([a, b]);
